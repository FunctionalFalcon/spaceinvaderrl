...
class Hyperparameters:
 """Hyperparameters for Dueling+Noisy DQN with Prioritized Replay.

 Mirrors Mnih 2015 defaults with Rainbow-era improvements:
 - Dueling DQN: V(s) and A(s,a) streams (Wang et al., 2016)
 - Noisy Nets: learned weight noise replaces ε-greedy (Fortunato et al., 2017)
 ...
 prio_alpha: float = 0.6  # Prioritization exponent: 0=uniform, 1=pure TD-error
 prio_beta: float = 0.4   # Initial IS weight exponent: 0=no correction, 1=full correction
 prio_beta_end: float = 1.0
 prio_beta_frac: float = 0.5  # Fraction of training over which beta anneals to prio_beta_end
 use_legacy_network: bool = False  # True = standard DQN head, False = Dueling+Noisy
